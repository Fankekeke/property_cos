  ...
  <a-modal v-model="show" title="抄表入库" @cancel="onClose" :width="800">
    ...
    <div>
      <a-row :gutter="20">
        ...
        <a-col :span="8">
          <a-form-item label='缴费月度' v-bind="formItemLayout">
            <a-month-picker placeholder="Select month" style="width: 100%" v-model="checkMonth">
...
export default {
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      if (!this.checkMonth) {
        this.$message.warning('请选择清算日期！')
        return false
      }
      console.log(moment(this.checkMonth).format('YYYY-MM-01'))
      this.$get('/cos/houses-info/housesConsumptionCheck', { housesId: this.housesId, date: moment(this.checkMonth).format('YYYY-MM-01') }).then((r) => {
        if (r.data.data.length !== 0) {
          ...
          this.propertyList.forEach(item => {
            ...
            paymentList.push({housesId: this.housesId, type: item.id, year: moment(this.checkMonth).format('YYYY'), month: moment(this.checkMonth).format('MM'), dosage, price})
          })
