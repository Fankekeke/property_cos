...
export default {
  ...
  methods: {
    ...
    setFormValues ({...rent}) {
      this.rowId = rent.id
      let fields = ['title', 'roomType', 'rentPrice', 'rentRemark', 'rentalRequest', 'rentType']
      let obj = {}
      this.setFormDeviceValues(rent)
      Object.keys(rent).forEach((key) => {
        if (key === 'roomPictures') {
          this.fileList = []
          this.imagesInit(rent['roomPictures'])
        }
        if (key === 'houseAddress') {
          this.houseAddress = rent[key]
        }
        if (key === 'roomType' || key === 'rentType') {
