...
<!--        <a-button type="primary" ghost @click="add">新增</a-button>-->
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          ...
          <a-icon v-if="record.createDate === null" type="alipay" @click="pay(record)" title="支 付"></a-icon>
        </template>
      </a-table>
    </div>
    <payment-view
...
export default {
  name: 'payment',
  components: {PaymentView, RangeDate},
  data () {
    return {
      advanced: false,
      paymentAdd: {
  ...
  methods: {
    pay (row) {
      let data = { outTradeNo: row.id, subject: `${row.year}年${row.month}月${row.propertyName}`, totalAmount: row.price, body: '' }
